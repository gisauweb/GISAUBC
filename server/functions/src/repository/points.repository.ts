...
export async function addPoints(user: User, pointsPayload: AddPointsModel) {
  ...
  if (
    user.past_activities &&
    user.past_activities[date] + pointsPayload.points > MAX_POINTS_LIMIT
  ) {
    points = MAX_POINTS_LIMIT - user.past_activities[date];
  }
  const pastActivities = handlePastActivities(
    points,
    user.past_activities,
    date
  );
