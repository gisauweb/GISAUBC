import { pgTable, unique, pgPolicy, uuid, text, timestamp, integer, varchar, numeric, pgEnum } from "drizzle-orm/pg-core"
...
export const profiles = pgTable("profiles", {
	...
	pgPolicy("Enable insert for authenticated users only", { as: "permissive", for: "insert", to: ["authenticated"], withCheck: sql`(( SELECT auth.uid() AS uid) = id)`  }),
]);
