...
const CustomSlide = (props) => {
  ...
  return (
    <Box className="flex flex-col justify-center text-center items-center bg-[#FFFDF5]">
      ...
      <span className="font-proxima-nova text-lg sm:text-xl px-3 py-5 font-extrabold">
        {title}
      </span>
      <p className="text-base px-3 sm:text-lg font-proxima-nova">
        {description}
      </p>
    </Box>
  );
...
export const ActivitiesSlider = ({ className }) => {
  const settings = {
    fade: true,
    // lazyLoad: true,
    infinite: true,
