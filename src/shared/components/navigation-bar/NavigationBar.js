...
export const NavigationBar = () => {
  ...
  const filteredPaths = pages
  .filter(page => page.hasLandingImage)
  .map(page => page.path);; 
          ...
          <Box className="flex mr-6 lg:mr-20">
            ...
            (
              <Link key={page.name} to={page.path} className="px-5 pt-3">
                <p className={`hover:underline underline-offset-8 decoration-2 font-oswald text-xl ${hasLandingImage ? "text-white" : "text-black"}`}>
                  {page.name}
                </p>
              </Link>
            ))}
