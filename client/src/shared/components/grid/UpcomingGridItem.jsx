    ...
    >
      ...
      <div>
        <Box className="flex flex-col mb-4 h-fit">
          <h4 className="mb-2 font-oswald font-semibold text-2xl">
            {item.title}
          </h4>
          <div className="flex flex-row flex-wrap gap-1 mb-2">
            {item.date && (
              <p className="px-3 py-1 bg-bgCream w-fit rounded-md text-sm">
                {item.date}
              </p>
            )}
            {item.time && (
          ...
          <p className="whitespace-pre-line text-base font-proxima">
            {item.caption}
          </p>
          {item.priceMember && (
          ...
          {item.isEvent ? (
            <>
              <Button
                ...
                handleClickButton={() =>
                  handleRegisterButton(item.registrationLink)
                }
              />
