  ...
  return (
    <div className="flex flex-col w-fit">
      <img
        src={data.image}
        alt=""
        className="aspect-square rounded-2xl object-cover w-full md:w-[440px] "
      />
      <div className="flex flex-col sm:flex-row justify-start sm:justify-between items-center">
        <div className="flex flex-col mt-3 w-full gap-y-2 sm:w-fit">
          <p className="font-oswald text-primary font-medium">
            {data.isEvent ? "EVENT" : "RANTANGAN"}
          </p>
          <p className="font-oswald font-semibold text-3xl">{data.title}</p>
          <div className="flex flex-row flex-wrap gap-1">
            {data.date && (
              <p className="text-sm bg-bgCream w-fit px-3 py-1 rounded-md">
                {data.date}
              </p>
            )}
            {data.time && (
        ...
        <Link to="/events" className="relative w-full sm:w-6/12 mt-5 sm:mt-0">
          <Button
            text={data.isEvent ? "Register Now" : "Order Now"}
        ...
        {eventDatas.length >= 1 ? (
          ...
          <div className="w-full text-center">
            ...
            <div className="flex flex-col md:flex-row justify-center gap-4 md:gap-10 mt-14">
              <Link
                to="/events"
                className="relative w-full lg:w-4/12 mt-5 sm:mt-0"
              >
                <Button
