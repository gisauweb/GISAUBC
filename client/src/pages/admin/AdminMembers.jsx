...
function PromoteModal({ memberName, onConfirm, onCancel }) {
	return (
		<div className='fixed inset-0 z-50 flex items-center justify-center bg-black/40'>
			<div className='bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4 border border-gray-200'>
				<div className='text-3xl mb-3 text-center'>⚠️</div>
				<h2 className='text-base font-bold text-gray-900 text-center mb-1'>Promote to Admin?</h2>
				<p className='text-sm text-gray-500 text-center mb-5'>
					<span className='font-medium text-gray-700'>{memberName}</span> will gain full admin access
					to this panel. This cannot be undone without another admin reversing it.
				</p>
...
function DetailPanel({ member, token, onClose, onSaved }) {
	...
	const [promoteModal, setPromoteModal] = useState(false); // pending role value when modal is open
	...
	const Select = ({ field, options, labelMap }) => (
		<select
			value={form[field] ?? ''}
			onChange={(e) => {
				if (field === 'role' && e.target.value === 'admin' && member.role !== 'admin') {
	...
	return (
		...
		{promoteModal && (
			<PromoteModal
				...
				onConfirm={() => { set('role', 'admin'); setPromoteModal(false); }}
