...
export default function Onboarding({ token, setAccount }) {
  ...
  const handleLogout = () => {
    logout({
      logoutParams: { returnTo: `${window.location.origin.toString()}` },
    });
  };
    ...
    <>
      <div className="h-screen flex flex-col justify-center items-center space-y-4">
        ...
        <div className="flex space-x-4">
          ...
          <Button
            text="Register"
            handleClickButton={() => setOpenDialog(!openDialog)}
          />
        </div>
      </div>
      <StudentIdDialog
